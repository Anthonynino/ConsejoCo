import { FaCalendarAlt, FaMapMarkerAlt, FaMoneyBillWave, FaEdit } from "react-icons/fa";
...
const ProjectCard = ({ project, onEdit }) => {
  ...
  return (
    <div className="card bg-base-100 border border-base-200 shadow-sm hover:shadow-md transition-all group relative">
      <button 
        onClick={onEdit}
        className="absolute top-4 right-4 btn btn-circle btn-ghost btn-sm opacity-0 group-hover:opacity-100 transition-opacity"
      >
        <FaEdit size={14} />
      </button>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3 pt-1">
            <div className="flex items-center gap-2 text-[10px] font-medium opacity-70">
              ...
              <span>${project.presupuesto_total?.toLocaleString()}</span>
            </div>
            <div className="flex items-center gap-2 text-[10px] font-medium opacity-70">
              <FaCalendarAlt className="text-primary" />
              <span className="truncate">{project.fecha_fin_estimada}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 text-[10px] font-medium opacity-70 border-t border-base-200/50 pt-2">
            ...
            <span className="truncate">{project.ubicacion_sector}</span>
          </div>

          <div className="text-[9px] opacity-40 italic">
            Iniciado: {project.fecha_inicio}
          </div>
        </div>
      </div>
    </div>
